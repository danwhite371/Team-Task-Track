...
import { tasks, allTaskTimes, newTaskData } from '../__tests__/data-test-util';
const {
...
mockFetch.mockImplementation(async (_url: string, options?: RequestInit) => {
  ...
  if ((requestBody.query as string).includes('mutation CreateTask')) {
    ...
    if (!taskId) {
      throw new Error('variables for GetTaskTimes has no taskId');
    }
    ...
    if (!id) {
      throw new Error('variables for StartTask has no id');
    }
    const task = tasks.find((t) => t.id == id);
    if (!task) {
      throw new Error(`Task was not found for id: ${id}`);
    }
    return Promise.resolve({
      ok: true,
    ...
    const id = requestBody.variables.id;
    if (!id) {
      throw new Error('variables for StopTask has no id');
    }
    const task = tasks.find((t) => t.id == id);
    if (!task) {
      throw new Error(`Task was not found for id: ${id}`);
